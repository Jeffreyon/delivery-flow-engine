...
- Added the first local BLN tenant-context and node bridge under `/api/v1/network`, then tightened it into a secure tenant-integration, membership, and node-assignment model with encrypted tenant API key storage, a compatibility-only `users.preferences.bln` mirror, and backend-only node-session minting at request time.
...
- Added the local BLN-backed custody facade under `/api/v1/handoffs` plus `/api/v1/deliveries/:id/handoff-status`, including handoff status, audit, initiate, retry, verify, dispute, and admin-only resolve routes over the existing tenant bridge.
- Corrected the BLN auth bridge so tenant data is now membership-scoped in this app, durable tenant integrations live in `bln_tenant_accounts`, local memberships and node assignments drive act-as rights, tenant API keys are stored only in encrypted backend storage, and normal runtime access now mints short-lived node sessions instead of relying on service-backed tenant exchange.
- Added admin-side tenant-member management under `/api/v1/network/users`, including local membership writes, node-assignment writes, and BLN node validation against the selected tenant before access state is persisted.
- Added explicit post-signup BLN provisioning under `/api/v1/network/provision-self` so the signup UI now creates the first tenant integration, owner membership, and default node assignment immediately after local account creation.
- Corrected the onboarding boundary so generic signup is local-only again, explicit workspace bootstrap now lives at `/api/v1/network/workspaces/bootstrap`, `/api/v1/network/provision-self` remains only as a compatibility alias, and the first user dashboard onboarding page now handles tenant creation separately from account creation.
- Added durable BLN tenant invitations plus authenticated invitation acceptance, so users can now join an existing tenant from the workspace onboarding page after local signup or login instead of bootstrapping a new tenant by default.
