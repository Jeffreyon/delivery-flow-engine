...
| Async architecture | BullMQ and `ioredis` are installed, shared queue config exists, `backend/worker.js` can boot a worker against `REDIS_URL`, and the live Railway project now has `worker` plus Redis-backed services in both environments | No named queues, processors, retries, or delivery jobs are implemented yet, and production currently resolves `REDIS_URL` through `Redis-nFa9` while staging uses `Redis` | Build later async slices on top of the live shared worker and queue bootstrap, then reconcile the Redis service-name drift without changing the worker env contract |
...
| Harness metadata truth | The migration runner is present, harness docs point at `.scaffold/project.json`, the Railway workflow includes a `worker` deploy target, and the live Railway project now has working `worker` plus Redis-backed deploy paths in both environments | The live project is configured, but worker behavior is still limited to idle boot because no queue processors exist yet, and production Redis naming currently drifts from the single-`Redis` metadata contract | Treat the repo metadata as the source of truth, keep the explicit live drift noted, and extend the topology through later queue-job slices instead of changing the service layout again |
