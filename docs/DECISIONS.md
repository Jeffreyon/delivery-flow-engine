...
| BLN client abstraction | The local backend must talk to `logistics-api` through an internal client layer | The PRD explicitly calls for a client layer between app routes and the external delivery API | Frontend callers should never see BLN service secrets, tenant API keys, or exchanged tenant tokens |
| Logistics client env contract | `LOGISTICS_API_URL`, `LOGISTICS_API_SERVICE_SECRET`, and optional `LOGISTICS_API_TIMEOUT_MS` | Slice 12 now needs a stable config surface before route facades land | Keep the contract small until retries, circuit breakers, or multi-region routing become necessary |
| Local-to-BLN auth bridge | Keep frontend auth local, then let the backend exchange trusted service assertions for tenant-scoped BLN access | `logistics-api` now supports `POST /api/tenant-auth/exchange` for the delivery-backend path | The first bridge can stay backend-only and service-secret based while the app decides how user-to-tenant binding works |
...
| Milestone 1 BLN actors | `admin` covers operator duties first, and broader non-admin BLN access is gated on a real local tenant-binding decision | The current repo only has `admin` and `user` roles, and there is still no local membership model for BLN tenants or nodes | Dedicated `operator` or `driver` local roles remain deferred until the bridge is real |
...
| The repo now has a live sibling BLN backend available | The backend-only client is landed, but no local context bridge or facade routes consume it yet | Use the new client as the only integration seam and move next to the local context bridge |
...
- Should BLN tenant or node binding live in `users.preferences` first, or does the repo need a dedicated local mapping table before non-admin BLN access becomes real?
