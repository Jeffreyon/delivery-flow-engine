...
| 2. Async platform slice | Define the queue runtime and deploy contract before job logic lands | BullMQ dependencies, shared queue config, `backend/worker.js`, and live `worker` plus Redis-backed Railway services now exist | No processors or named queues exist yet, and the live Railway Redis service name still drifts between `staging` and `production` | Keep the single-worker async baseline and layer real jobs on top of it in later slices while leaving the Redis drift explicit | `docs/ARCHITECTURE.md`, `docs/DECISIONS.md`, `docs/IMPLEMENTATION_PLAN.md`, `docs/MIGRATION_WORKFLOW.md`, `.github/workflows/railway-deploy.yml`, `.scaffold/project.json`, `backend/src/core/queue`, `backend/worker.js` | 1 | None; the monolithic worker baseline is now the locked starting point |
